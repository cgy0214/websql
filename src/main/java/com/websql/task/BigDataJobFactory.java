...
public class BigDataJobFactory implements Task {
    ...
    private static final ConcurrentHashMap<Long, ReentrantLock> TASK_LOCKS = new ConcurrentHashMap<>();
    ...
    public void execute() {
        ...
        ReentrantLock taskLock = TASK_LOCKS.computeIfAbsent(bigDataTaskModel.getId(), k -> new ReentrantLock());
        boolean acquired = false;
        try {
            acquired = taskLock.tryLock();
            if (!acquired) {
                log.warn("{}-已在运行中，跳过本次执行", bigDataTaskModel.getTaskName());
                return;
            }
            ...
            if (acquired && taskLock.isHeldByCurrentThread()) {
                taskLock.unlock();
            }
        }
    }
